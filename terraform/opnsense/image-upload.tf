...
resource "null_resource" "image_upload" {
  ...
  provisioner "local-exec" {
    when    = create
    command = <<-EOF
      curl -Lo opnsense.img.bz2 https://www.mirrorservice.org/sites/opnsense.org/releases/${self.triggers.opnsense_version}/OPNsense-${self.triggers.opnsense_version}-serial-amd64.img.bz2
