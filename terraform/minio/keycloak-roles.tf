...
resource "keycloak_role" "minio_console_admin" {
  realm_id    = data.terraform_remote_state.keycloak.outputs.primary_realm_id
...
resource "keycloak_group_roles" "minio_console_admin" {
  realm_id = data.terraform_remote_state.keycloak.outputs.primary_realm_id
  group_id = data.keycloak_group.system_admins.id

  role_ids = [
    keycloak_role.minio_console_admin.id
  ]

  exhaustive = false
}

resource "keycloak_openid_user_client_role_protocol_mapper" "minio_claim_mapper" {
  realm_id                    = data.terraform_remote_state.keycloak.outputs.primary_realm_id
  client_id                   = keycloak_openid_client.minio.id
  name                        = "role-mapper"
  claim_name                  = "roles"
