resource "random_password" "penpot_db_username" {
  length  = 16
  numeric = false
  special = false
  upper   = false
...
resource "random_password" "penpot_db_password" {
  length  = 16
  numeric = true
  special = false
  upper   = true
...
resource "kubernetes_secret" "db_credentials" {
  ...
  data = {
    username = random_password.penpot_db_username.result
    password = random_password.penpot_db_password.result
  }
}
